...
@Component
public class AsuntosEntradosModelAssembler
		implements RepresentationModelAssembler<AsuntosEntrados, EntityModel<AsuntosEntradosDto>> {
	...
	public EntityModel<AsuntosEntradosDto> toModel(AsuntosEntrados entity) {
		...
		dto.setExpedientes(listaExpedienteExportDto(entity.getExpedientes()));
	...
	private AsuntosEntradosDto convertirPojoADto(AsuntosEntrados asuntos) {
		...
		if (asuntos.getNota() != null) {
			asuntosDto.setNota(linkTo(methodOn(AsuntosEntradosRestController.class).descargarNota(asuntos.getId()))
					.toUri().toString());
		}
	...
	private List<EntityModel<ExpedienteExportDto>> listaExpedienteExportDto(List<Expediente> expedientes) {

		List<EntityModel<ExpedienteExportDto>> expedientesReducidos = expedientes.stream()
				.map(e -> (Expediente) e) // ⬅️ cast explícito
				.map(exp -> {
					ExpedienteExportDto reducido = convertirAExpedienteReducido(exp);
					return EntityModel.of(reducido,
							linkTo(methodOn(ExpedienteRestController.class).obtener(exp.getId())).withSelfRel());
				}).collect(Collectors.toList());
		return expedientesReducidos;
	}
}
